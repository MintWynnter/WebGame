import { Game, RollbackWrapper, } from "netplayjs/src/index";
...
function rectOverlap(aLeft, aRight, aTop, aBottom, bLeft, bRight, bTop, bBottom) {
...
export class Pong extends Game {
    ...
    tick(playerInputs) {
        ...
        for (const [player, input] of playerInputs.entries()) {
            const direction = (input.keysHeld["ArrowDown"] ? 1 : 0) +
            ...
            if (player.getID() == 0) {
                if (paddlePos)
                    this.leftPaddle = paddlePos;
                else
                    this.leftPaddle += direction * PADDLE_MOVE_SPEED * dt;
            }
            else if (player.getID() == 1) {
                if (paddlePos)
                    this.rightPaddle = paddlePos;
                else
        ...
        if (rectOverlap(this.ballPosition[0], this.ballPosition[0] + BALL_WIDTH, this.ballPosition[1], this.ballPosition[1] + BALL_HEIGHT, LEFT_PADDLE_X, LEFT_PADDLE_X + PADDLE_WIDTH, this.leftPaddle, this.leftPaddle + PADDLE_HEIGHT)) {
            let offset = (this.ballPosition[1] +
                BALL_HEIGHT / 2 -
        ...
        if (rectOverlap(this.ballPosition[0], this.ballPosition[0] + BALL_WIDTH, this.ballPosition[1], this.ballPosition[1] + BALL_HEIGHT, RIGHT_PADDLE_X, RIGHT_PADDLE_X + PADDLE_WIDTH, this.rightPaddle, this.rightPaddle + PADDLE_HEIGHT)) {
            let offset = (this.ballPosition[1] +
                BALL_HEIGHT / 2 -
    ...
    draw(canvas) {
        ...
        // Draw ball.
        ctx.fillStyle = "white";
        ctx.fillRect(this.ballPosition[0], this.ballPosition[1], BALL_WIDTH, BALL_HEIGHT);
        // Draw scores.
        ctx.font = "40px Arial";
        ctx.textAlign = "center";
        ctx.fillText(this.leftScore.toString(), PONG_WIDTH * 0.3, PONG_HEIGHT * 0.2);
        ctx.fillText(this.rightScore.toString(), PONG_WIDTH * 0.7, PONG_HEIGHT * 0.2);
    }
}
